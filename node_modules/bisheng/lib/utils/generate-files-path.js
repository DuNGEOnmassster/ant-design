...
module.exports = function generateFilesPath(routes, markdown) {
  ...
  var filesPath = R.chain(function (item) {
    if (hasParams(item.path)) {
      ...
      var processedCompleteRoutes = Object.keys(dataSet).map(function (key) {
        ...
        return {
          path: path,
          dataPath: dataPath,
          isFolder: _typeof(dataSet[key]) === 'object' && !('zh-CN' in dataSet[key] || 'en-US' in dataSet[key])
        };
      });
    ...
    return !item.isFolder && ["".concat(item.path, ".html")];
  }, flattenedRoutes).filter(function (c) {
    return c;
  });
